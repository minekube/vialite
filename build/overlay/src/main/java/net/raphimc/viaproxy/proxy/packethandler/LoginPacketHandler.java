...
public class LoginPacketHandler extends PacketHandler {
    ...
    public boolean handleC2P(Packet packet, List<ChannelFutureListener> listeners) throws GeneralSecurityException {
        if (packet instanceof C2SLoginHelloPacket loginHelloPacket) {
            ...
            if (ViaProxy.getConfig().isProxyOnlineMode() && !ViaProxy.EVENT_MANAGER.call(new ShouldVerifyOnlineModeEvent(this.proxyConnection)).isCancelled()) {
                this.proxyConnection.getC2P().writeAndFlush(new S2CLoginHelloPacket("", keyPair().getPublic().getEncoded(), this.verifyToken, true)).addListener(ChannelFutureListener.FIRE_EXCEPTION_ON_FAILURE);
            } else {
            ...
            if (loginKeyPacket.encryptedNonce != null) {
                if (!Arrays.equals(this.verifyToken, CryptUtil.decryptData(keyPair().getPrivate(), loginKeyPacket.encryptedNonce))) {
            ...
            final SecretKey secretKey = CryptUtil.decryptSecretKey(keyPair().getPrivate(), loginKeyPacket.encryptedSecretKey);
            ...
            HTTP_EXECUTOR.submit(() -> {
                ...
                try {
                    final String serverHash = new BigInteger(CryptUtil.computeServerIdHash("", keyPair().getPublic(), secretKey)).toString(16);
    ...
    private static KeyPair keyPair() {
        ...
        synchronized (LoginPacketHandler.class) {
            current = keyPair;
            if (current == null) {
                current = CryptUtil.generateKeyPair();
                keyPair = current;
            }
            return current;
        }
